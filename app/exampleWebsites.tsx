...
function RisosEnterprisesWebsite() { 
    const handleOver = () =>  document.getElementById('risosCover')?.classList.add('animate');
    const handleLeave = () => document.getElementById('risosCover')?.classList.remove('animate');
    return (
        <button onClick={() => window.location.href = 'http://www.risosenterprises.com'} onMouseEnter={() => handleOver()} onMouseLeave={() => handleLeave()} className="website p-2 backdrop-blur-xl rounded-3xl relative transition-all duration-150 hover:scale-[102%] hover:-translate-y-2 overflow-hidden">
...
function CarMarket() { 
    const handleOver = () => document.getElementById('carCover')?.classList.add('animate');
    ...
    function CarkMarketCover() { 
        return ( 
            <div id="carCover" className="bg-[rgba(0,0,0,0.5)] flex flex-col justify-center items-center w-full h-full absolute top-0 translatexfull left-0 z-50">
                <p className="text-white text-2xl text-center py-4 font-bold">MovieDatabases</p>
